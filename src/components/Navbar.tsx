...
interface NavbarProps {
  activeTab?: 'planner' | 'history' | 'destinations' | 'dashboard';
  onNavigate: (tab: any) => void;
  showSearch?: boolean;
  userRole?: 'user' | 'admin';
}

export function Navbar({ activeTab, onNavigate, showSearch = false, userRole = 'user' }: NavbarProps) {
  const tabs = [
    { id: 'planner', label: '规划专家' },
    { id: 'history', label: '历史行程' },
    { id: 'destinations', label: '热门目的地' },
    ...(userRole === 'admin' ? [{ id: 'dashboard', label: '系统概览' }] : []),
  ];

  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 w-[calc(100%-48px)] max-w-[1200px] z-50">
      <div className="bg-white border border-gray-200 rounded-[32px] shadow-bento px-8 h-20 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-10 h-10 rounded-full border border-gray-200 overflow-hidden cursor-pointer active:scale-95 transition-transform">
            <img 
